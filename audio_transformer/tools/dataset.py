...
#
#    csv_path = 'data/csv_files/{}.csv'.format(data_type)
#    audio_dir = 'data/waveforms/{}/'.format(data_type)
#    hdf5_path = 'data/hdf5s/{}/'.format(data_type)
#
# data_type :  train, val, test
# csv_path : data/csv_file/train.csv
# audio_dir : data/waveforms/train/
# hdf5_path = data/hdf5s/train/

def pack_wavs_to_hdf5_fine(data_type, csv_path, audio_dir, hdf5_path):
    ...
    with h5py.File(hdf5_path+'{}.h5'.format(data_type), 'w') as hf:
        hf.create_dataset('audio_name', shape=(audio_nums,), dtype='S20')
        hf.create_dataset('waveform', shape=(audio_nums, audio_length), dtype=np.float32)
        if data_type == 'train':
            hf.create_dataset('caption', shape=(audio_nums,), dtype=h5py.special_dtype(vlen=str))
    ...
    logger.info(f'Packed {data_type} set to {hdf5_path} using {time.time() - start_time} s.')
