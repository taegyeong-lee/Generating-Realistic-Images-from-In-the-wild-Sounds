...
def greedy_decode2(model, src, sos_ind=0, eos_ind=9, max_len=30):
...
def greedy_decode(model, src, sos_ind=0, eos_ind=9, max_len=30):
    ...
    with torch.no_grad():
        ...
        ys = torch.ones(batch_size, 1).fill_(sos_ind).long().to(src.device)
        prob_list = torch.zeros(batch_size, 1).float().to(src.device)
        ...
        for i in range(max_len):
            ...
            next_word = torch.argmax(prob, dim=1)
            next_word_prob, _ = torch.max(prob, dim=1)

            next_word_prob = next_word_prob.unsqueeze(1)
            next_word = next_word.unsqueeze(1)

            ys = torch.cat([ys, next_word], dim=1)
            prob_list = torch.cat([prob_list, next_word_prob], dim=1)

    return ys, prob_list

def greedy_decode_gumbel(model, src, sos_ind=0, eos_ind=9, max_len=30):
