...
class AudioCapsDataset(Dataset):
    ...
    def __init__(self, config):
        ...
        self.h5_path = config.path.hdf5_train_path
        vocabulary_path = config.path.vocabulary
...
class AudioCapsEvalDataset(Dataset):
    ...
    def __init__(self, split, config):
        ...
        if split == 'val':
            self.h5_path = config.path.hdf5_val_path
        elif split == 'test':
            self.h5_path = config.path.hdf5_test_path
        with h5py.File(self.h5_path, 'r') as hf:
            self.audio_names = [audio_name.decode() for audio_name in hf['audio_name'][:]]
